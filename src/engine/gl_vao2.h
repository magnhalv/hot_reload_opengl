...
struct GLBufferElementDescription2 {
  u32 binding_index;
  i32 size;
  i32 offset_in_bytes;
  i32 stride_in_bytes;
...
struct GLBuffer2 {
  ...
  GLbitfield flags;
  GLBufferElementDescription2 description[Max_Num_Descriptions];
  i32 num_descriptions;
};

struct GLElementBuffer2 {
  u32 handle;
  GLsizeiptr size;
};
...
struct GLVao2 {
  ...
  u32 handle;
  GLBuffer2 buffers[Max_Buffers];
  GLElementBuffer2 el_buffer;
  u32 num_buffers;
  ...
  auto add_buffer_desc(u32 buffer_idx, GLBufferElementDescription2 desc) -> void;
